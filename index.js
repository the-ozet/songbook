...
function initStore(restoreUserState) {
  return (restoreUserState && storeIsValid(SAVED_STORE)) ? SAVED_STORE : DEFAULT_STORE;
}
...
function Chapter(props) {
  ...
  function displayChapterTitle() {
    if (chapterTitle && sectionIndex === 0) {
      const number = (chapterType)? null
        : <span className="chapter-number">Chapter { chapterIndex }</span>
      return <h2>
        { number }
  ...
  return <div className="chapter" data-slug={ fileName }>
    <div className="mask" />
...
function TableOfContents(props) {
  ...
  return <CSSTransition unmountOnExit in={ inProp } timeout={ 300 } classNames="table-of-contents">
    <div className="table-of-contents">
      <header>Table of Contents</header>
      { renderList('cover') }
      { renderList('chapter', 'Chapters') }
    </div>
  </CSSTransition>
} 

function Header(props) {
  return <div className="chapter-header">
    <header className="chapter-header">{ props.chapterTitle }</header>
  </div>
}

function Pagination(props) {
  const { sectionIndex, chapterIndex, muted, dispatch } = useContext(StoryContext);
  ...
  function up() { scrollBy(-window.innerHeight) }

  function down() { scrollBy(window.innerHeight) }

  function toggleTOC() { setShowTOC(!showTOC) }
  ...
  return <div className="pagination">
    ...
    <button onClick={ prev } className={ inactiveClass(prev) }>Back</button>
    <div className="pager">
      <button onClick={ up }>⤒</button>
      <button onClick={ down }>⤓</button>
    </div>
    <button onClick={ next } className={ inactiveClass(next) }>Next</button>
...
function Section(props) {
  ...
  const renderLines = lines.map((l, i) => {
    ...
    switch(type) {
      case 'title':
        return <h3 { ...lineProps } className="section-title">{ value }</h3>
      case 'video-bg':
        return <div className="video-bg">
          <video { ...lineProps } src={ `video/${value}` } loop={ true } muted={ true } controls={ false } autoPlay={ true } />
        </div>
      case 'image-bg':
        return <div { ...lineProps } className="image-bg" style={{ backgroundImage: `url(images/${value})` }} />
      case 'audio-bg':
        return <audio { ...lineProps } ref={ audioElem }>
          <source src={ `audio/${value}` } type="audio/mp3" loop={ true }/>
        </audio>
      case 'image-bg-grainy':
        return <div { ...lineProps } className="image-bg grainy" style={{ backgroundImage: `url(images/${value})` }} />
      case 'image':
        return <figure className="image-inset" { ...lineProps }>
          <img src={ `images/${value}` } />
        </figure>
      case 'video':
        return <figure className="video-inset" { ...lineProps }>
          <video src={ `video/${value}` } loop={ true } muted={ muted } controls={ true } autoPlay={ true } />
        </figure>
      case 'catalog':
        // return null;
        // TK TK TK 
        ...
        return <table className="catalog">
          <thead>
            <tr>
              <th>Tape#</th>
              <th>Date</th>
  ...
  return <CSSTransition unmountOnExit in={ inProp } timeout={ transitionDuration } classNames="section">
    <div>{ renderLines }</div>
  </CSSTransition>
}
